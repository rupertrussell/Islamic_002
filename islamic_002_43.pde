...
import processing.pdf.*;  // Used to save to PDF
...
void setup() {
  ...
  // beginRecord(PDF, "islamic_002.pdf"); // Saves to a PDF uncomment endrecord at bottom of code as wells
}
...
void draw() {
  ...
  // Draw the inner most circle points
  //drawInner();
...
void drawStarOne() {
  ...
  strokeWeight(3);
  for (int x=0; x < 23; x=x+1) {
    circle(starOneX[x], starOneY[x], 10);
  }
}
...
void drawStarTwo() {
  ...
  for (int x=0; x < 24; x=x+1) {
    circle(starTwoX[x], starTwoY[x], 10);
  }
}
...
void cutStar() {
  strokeWeight(3);
  stroke(255, 0, 0);
  for ( int x=1; x < 23; x = x +2) {

    line(starTwoX[x], starTwoY[x], innerX[x-1], innerY[x-1]);
    line(starTwoX[x], starTwoY[x], innerX[x-1], innerY[x-1]);
  }
  ...
  for ( int x=1; x < 23; x = x +2) {
    stroke(255, 0, 0);
    line(starTwoX[x+2], starTwoY[x+2], innerX[x+3], innerY[x+3]);
  }

  // last line  
  stroke(255, 0, 0);
  line(starTwoX[23], starTwoY[23], innerX[22], innerY[22]);

  // First line  
  stroke(255, 0, 0);
  line(starTwoX[1], starTwoY[1], innerX[2], innerY[2]);
}
